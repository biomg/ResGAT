...
for i in range(5):
    ...
    GATMO = GraphNet(num_node_features=test_data.num_node_features).to(device)
    GATMO.load_state_dict(torch.load(model_path, map_location=device))
    GATMO.eval()

    with torch.no_grad():
        preds_test = GATMO(test_data.x, test_data.edge_index)
